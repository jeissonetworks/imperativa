...
const inmobiliaria = {
  ...
  alquilar: function(id){
    for (let i = 0; i < this.departamentos.length; i++) {
      if (id === this.departamentos[i].id) {
        this.departamentos[i].disponible = !this.departamentos[i].disponible
        return `departamento con id: ${id} está alquilado`;
      }
    }
  },


// Agregar un método filtrarPetFriendly que permita filtrar los departamentos que admiten mascotas.
...
filtrarPetFriendly: function() {
   ...
   for (let i = 0; i < this.departamentos.length; i++) {
     if(this.departamentos[i].aceptaMascotas === true){
       allowPets.push(this.departamentos[i])
     }
   }
   return allowPets;
  },

// Agregar un método rebajasPorNoAlquiler que realice un descuento del 10% a los que no estén alquilados aún para una promoción. 
...
rebajasPorNoAlquiler: function(){
    ...
    let valorTotal = 0.9;
    for (let i = 0; i < aptosDisponibles.length; i++) {
     aptosDisponibles[i].precioAlquiler =
     valorTotal * aptosDisponibles[i].precioAlquiler;
    }
    return aptosDisponibles;
  },
...
buscarPorPropietarios: function(nombrePropietario){
    let departamentoDePropietario = []
    for (let i = 0; i < this.departamentos.length; i++) {
      if (this.departamentos[i].propietarios.includes(nombrePropietario)) {
        departamentoDePropietario.push(this.departamentos[i]);
      }
    }
    return departamentoDePropietario
  },
...
console.log(v, oo + "alquilar");
// Ejecución aquí
console.log(inmobiliaria.alquilar(5));
console.log(o);

console.log(v, oo + "filtrarPetFriendly");
inmobiliaria.listarDepartamentos(inmobiliaria.filtrarPetFriendly());

console.log(v, oo + "rebajasPorNoAlquiler ");
inmobiliaria.listarDepartamentos(inmobiliaria.rebajasPorNoAlquiler());

console.log(v, oo + "buscarPorPropietarios  ");
inmobiliaria.listarDepartamentos(inmobiliaria.buscarPorPropietarios('Luis Perez'));
