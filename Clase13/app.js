...
const banco = {
  ...
  deposito: function (titular, cantidadADepositar) {
    let totalADepositar =
      banco.consultarCliente(titular).saldoEnPesos + cantidadADepositar;
    return `Depósito realizado, su nuevo saldo es: ${totalADepositar}`;
  },
  extraccion: function (titular, cantidadAExtraer) {
    let saldo = banco.consultarCliente(titular).saldoEnPesos - cantidadAExtraer;
    if (saldo > 0) {
